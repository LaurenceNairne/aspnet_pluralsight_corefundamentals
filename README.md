...
- Using the command `dotnet ef migration add`, we'll add migration code to our project
- Entity Framework generates this by looking at an existing database if one exists, or creating one from scratch if it does not
- It looks at our DbContext and our entities
- It figures out what schema changes are needed to make the app function correctly
- If we update our entities and/or add more, we can add further migrations to the project to ensure the database schema remains in sync
- Using the command `dotnet ef database update` applies any migrations to update our database - naturally this only works if there are migrations in our project
- EF passes on any data validation requirements like character limits and required
- By default EF makes an `Id` property the primary key in the database
- `ValueGenerationStrategy` makes `id` column an `IdentityColumn` which prompts SQL Server to auto-generate IDs when a new entry is added to the table.
